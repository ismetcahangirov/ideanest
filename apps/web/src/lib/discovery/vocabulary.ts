 ...
 *
 *   `q` (free text)                  #43 — no search vector exists
...
export const SORT_VALUES = ['newest', 'ending_soon', 'most_funded', 'most_backed', 'popularity'] as const;
 ...
 * open on the same handful of campaigns for everybody for ever. Because it is
 * the default it is also the one sort that is never written to the URL — see
 * `filters.ts`.
 */
